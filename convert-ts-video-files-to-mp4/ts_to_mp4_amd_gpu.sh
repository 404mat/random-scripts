...
# Check if FFmpeg supports AMD AMF
if ! ffmpeg -encoders | grep -q h264_amf; then
    echo "FFmpeg does not support AMD AMF. Please ensure FFmpeg is built with AMF support."
    exit 1
fi
...
for ts_file in *.ts; do
    ...
    # Run FFmpeg with AMF to convert .ts to .mp4
    echo "Converting $ts_file to $output_file using AMD AMF..."
    ffmpeg -i "$ts_file" -c:v h264_amf -preset fast -c:a aac "$output_file"

    echo "Converted: $output_file"
done

echo "All .ts files have been converted to .mp4 using AMD AMF."
