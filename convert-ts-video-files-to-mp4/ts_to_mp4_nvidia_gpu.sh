...
if ! ffmpeg -encoders | grep -q h264_nvenc; then
    echo "FFmpeg does not support NVIDIA NVENC. Please ensure FFmpeg is built with NVENC support."
    exit 1
fi
...
for ts_file in *.ts; do
    ...
    # Run FFmpeg with NVENC to convert .ts to .mp4
    echo "Converting $ts_file to $output_file using NVIDIA NVENC..."
    ffmpeg -i "$ts_file" -c:v h264_nvenc -preset fast -c:a aac "$output_file"

    echo "Converted: $output_file"
done

echo "All .ts files have been converted to .mp4 using NVIDIA NVENC."
